...
$sql = "SELECT * FROM ".$SETTINGS["data_table"]." WHERE `date` LIKE '".$cYear."%'";
$sql_result = mysql_query ($sql, $connection ) or die ('request "Could not execute SQL query" '.$sql);
while ($row = mysql_fetch_assoc($sql_result)) {
	$events[$row["date"]]["name"] = $row["name"];
...
?>
  ...
  <tr>
      ...
      <td colspan="5" class="cMonth" style="background-color:coral;"><?php echo date("F, Y",strtotime($cYear."-".$cMonth."-01")); ?></td>
